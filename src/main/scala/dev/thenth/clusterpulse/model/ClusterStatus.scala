...
case class ClusterStatus(
  ...
  /**
   * Shard balance score: 0.0 = perfectly balanced, 1.0 = maximally imbalanced.
   * Computed as the coefficient of variation (stddev / mean) of per-node entity counts,
   * clamped to [0.0, 1.0]. Returns 0.0 when there are fewer than 2 shard-hosting nodes
   * or no entities. Nodes with no shards (non-hosting nodes) are excluded from the
   * calculation so they don't penalize the balance score.
   */
