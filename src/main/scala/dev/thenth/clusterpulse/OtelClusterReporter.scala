...
/**
 * Bridges cluster-pulse metrics into OpenTelemetry by registering
 * `ObservableLongGauge` callbacks on the OTel `Meter`.
 *
 * Call [[update]] on every reporting tick to refresh the latest
 * [[ClusterStatus]] snapshot; the OTel SDK reads the gauges on its own
 * export interval.
 *
 * @param otel               the OpenTelemetry instance to register metrics with
 * @param splitBrainDetector optional detector whose state is exported as a gauge
 * @param history            optional history buffer whose size and delta are exported
 */
class OtelClusterReporter(
  ...
  meter.gaugeBuilder("cluster.pulse.shard.region.count").ofLongs()
  ...
  meter.gaugeBuilder("cluster.pulse.node.status").ofLongs()
  ...
  splitBrainDetector.foreach { sbd =>
    meter.gaugeBuilder("cluster.pulse.split_brain.detected").ofLongs()
  ...
  history.foreach { h =>
    meter.gaugeBuilder("cluster.pulse.node.count.delta").ofLongs()
      .setDescription("Node count change over the history window")
      .buildWithCallback(m => m.record(h.nodeCountDelta.toLong))

    meter.gaugeBuilder("cluster.pulse.history.size").ofLongs()
