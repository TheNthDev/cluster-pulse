...
/** Thread-safe rolling history buffer that keeps the last N cluster status snapshots and emits structured change events
  * by diffing consecutive snapshots.
  *
  * @param maxSize
  *   maximum number of snapshots to retain (defaults to config value or 60)
  */
class ClusterHistory(val maxSize: Int = 60) {
  ...
  /** Record a new snapshot. Automatically diffs against the previous snapshot and appends any resulting events to the
    * event log.
    */
  ...
  /** Node count delta: difference between the latest and oldest snapshot's node count. Returns 0 if fewer than 2
