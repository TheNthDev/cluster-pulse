...
/** Typed accessor for all `cluster-pulse.*` configuration values.
  *
  * Reads from the supplied Typesafe `Config` and exposes each setting as a strongly-typed Scala value (durations,
  * timeouts, thresholds, etc.).
  *
  * @param config
  *   the root application config (must contain a `cluster-pulse` section)
  */
