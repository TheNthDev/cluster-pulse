...
/**
 * Heuristic split-brain detector that tracks cluster state across snapshots
 * and flags potential network partitions.
 *
 * Detection heuristics:
 *  1. **Unreachable majority**: more than half the nodes are unreachable.
 *  2. **Rapid membership change**: a large fraction of nodes appeared or disappeared
 *     between consecutive snapshots (configurable threshold).
 *  3. **Leader gap**: the cluster has nodes but none report a "Up" status
 *     (all are Joining, WeaklyUp, Leaving, Exiting, Down, or Unreachable).
 *
 * This is a **complement** to a proper Split Brain Resolver (SBR) — it detects
 * symptoms and exposes a gauge, but does not take corrective action.
 *
 * @param membershipChangeThreshold fraction (0.0–1.0) of nodes that must change
 *                                  between snapshots to trigger the rapid-change heuristic.
 */
